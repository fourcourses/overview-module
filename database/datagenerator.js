...
const generateData = async (callback) => {
    ...
    images.write('listingId|imgUrl|caption\n');
    ...
    for(let id = 1; id <= 1e7; id++) {
        ...
        const post = `${id}|${faker.random.word()}|${JSON.stringify(faker.lorem.paragraphs(3))}|${faker.random.number({min: 1, max: 999})}|${faker.random.number({min: 2, max: 5, precision: 0.01})}|${faker.random.number({min: 3, max: 10})}\n`
        ...
        for (let idx = 0; idx < randPhotoNum; idx++) {
            if (!images.write(`${id}|${faker.image.food()},${faker.random.word() + faker.date.past()}\n`)) {
        ...
        for (let idx = 0; idx < randTagNum + 5; idx++) {
            if (!tags.write(`${id}|${faker.random.word()}\n`)) {
        ...
        for (let idx = 0; idx < randCuiNum; idx++) {
          ...
          if (!types.write(`${id}|${cuisine[randCuisineIndex]}\n`)) {
